...
class PNS_Partners_Widget extends \Elementor\Widget_Base {

	public function get_name() {
		return 'pns_partners_gallery';
	}

	public function get_title() {
		return esc_html__( 'Logo Carousel', 'pns-addons-for-elementor' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'section_header',
			[
				'label' => esc_html__( 'Section Header', 'pns-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( "Trusted by 200+ of the world's top brands", 'pns-addons-for-elementor' ),
				'label_block' => true,
			]
		);
		...
		$this->start_controls_section(
			'section_gallery',
			[
				'label' => esc_html__( 'Partner Logos', 'pns-addons-for-elementor' ),
				'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$this->add_control(
			'brand_logos',
			[
				'label' => esc_html__( 'Add Brand Logos', 'pns-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'marquee',
				'options' => [
					'marquee' => esc_html__( 'Infinite Scrolling Marquee', 'pns-addons-for-elementor' ),
					'grid' => esc_html__( 'Static Responsive Grid', 'pns-addons-for-elementor' ),
				],
			]
		);
		...
		$this->add_control(
			'marquee_speed',
			[
				'label' => esc_html__( 'Marquee Scroll Speed (Seconds)', 'pns-addons-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 10,
		...
		$this->add_control(
			'enable_hover',
			[
				'label' => esc_html__( 'Enable Hover Effect', 'pns-addons-for-elementor' ),
		...
		$this->add_control(
			'logo_opacity',
			[
				'label' => esc_html__( 'Logo Opacity (0-1)', 'pns-addons-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0.1,
		...
		$this->add_control(
			'logo_hover_opacity',
			[
				'label' => esc_html__( 'Logo Hover Opacity (0-1)', 'pns-addons-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0.1,
		...
		$this->add_responsive_control(
			'logo_spacing_h',
			[
				'label' => esc_html__( 'Horizontal Spacing', 'pns-addons-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
		...
		$this->add_responsive_control(
			'logo_spacing_v',
			[
				'label' => esc_html__( 'Vertical Row Spacing', 'pns-addons-for-elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'vh' ],
				'range' => [
					'px' => [
	...
	protected function render() {
		...
		if ( empty( $logos ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div style="text-align: center; padding: 40px; border: 2px dashed #ccc; color: #777; font-family: sans-serif; border-radius: 8px;">' . esc_html__( 'Please select brand logo images in the Elementor settings panel.', 'pns-addons-for-elementor' ) . '</div>';
			}
			return;
		}
		?>
				...
				<div class="brands-static-grid">
				  <?php foreach ( $logos as $logo ) : ?>
					<div class="brand-item">
					  <img src="<?php echo esc_url( $logo['url'] ); ?>" class="brand-logo-img" alt="<?php echo esc_attr__( 'Brand Logo', 'pns-addons-for-elementor' ); ?>" draggable="false" />
					</div>
				  <?php endforeach; ?>
				</div>
			  <?php else : ?>
